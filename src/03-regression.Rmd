...
[@kirck2008pearson]. O coeficiente de Pearson de amostras populacionais 
pertecentes a duas variáveis pode ser definido pela 
Equação \@ref(eq:corr-pearson).
...
Os valores do coeficiente de Pearson variam entre -1 e 1, de tal forma que 
quanto mais próximos desses extremos, melhor correlacionado estão as variáveis. 
A Figura \@ref(fig:scatterCorrelations) mostra alguns exemplos com gráficos de 
disperssão de variáveis com diferentes correlações.
...
representam o intercepto e inclinação da reta, respectivamente.
...
O Métodos dos quadrados ordinários (MQO) ou método dos mínimos quadrados (MMQ)
busca encontrar o melhor valor para os coeficientes, de tal forma que a 
diferença absoluta entre o valor real e o predito pela função seja o menor para
todos os pontos. A Figura \@ref{fig:ols} mostra um exmplo de regressão linar 
utilizando o MQO.
