...
```{r clustering-book, echo=FALSE, fig.align="center", fig.show = "hold", out.width="100%", fig.cap='Antes e depois da organização da prateleira de livros em grupos baseados em gênero.'}
...
são separados é chamado de **técnicas** ou **métodos** de **agrupamento**. De 
...
descoberta de padrões e na criação de novos rótulos a partir dos grupos gerados. 
</p>
...
```{r agrup-exemplo, echo=FALSE, fig.align="center", fig.show = "hold", out.width = "70%", fig.cap='Exemplo de dois agrupamentos em um plano com duas dimensões (X,Y). Observam-se 3 grupos em a) e 8 grupos em b). Adaptado de @esling2012time '}
# refazer essa figura
...
```{r centroid-based, echo=FALSE, fig.align="center", fig.show = "hold", out.width="100%", fig.cap='Exemplo de um agrupamento beaseado em partição. As linhas pretas demarcam as partições e as formas geométricas preenchidas os centróides. Adaptado de @google2020'}
...
**aglomerativos** ou **divisivos**, baseado no modo em que sua decomposição hierárquica é formada. No método **aglomerativo**, também conhecido como 
AGNES (*Agglomerative Nesting*), considera-se, inicialmente, que cada objeto seja seu próprio grupo, e a cada iteração, os dois grupos mais parecidos são juntados, desta forma, formando um novo um grupo, esse processo é repetido até que todos os objetos estejam em um único grupo. De modo oposto, o método **divisivo**, também conhecido como DIANA (*Divise Analysis*), considera-se que todos objetos pertencem a um único grupo, e posteriormente, divide-se o único grupo até obter-se um grupo para cada objeto ou até que um critério de párada seja atendido [@han2011data].
</p>

```{r hierarquical-based, echo=FALSE, fig.align="center", fig.show = "hold", out.width="100%", fig.cap='Exemplo do processo de criação de grupos baseado nas técnicas de agrupamento hierarquicas. - Fonte: @han2011data'}
...
<p align="justify">
Uma técnica que auxilia na interpretação dos agrupamentos hierárquicos é o dendrograma, apresentado na Figura \@ref(fig:dend-based), representado em forma de árvore, nele é possível obter informações sobre a estrutura do agrupamento realizado. No nível 0 é possível observar os objetos em seus próprios grupos, e, conforme o nível da árvore aumenta em relação a sua raiz, menor são as similaridades entre os objetos do mesmo grupo [@han2011data]. 
</p>

```{r dend-based, echo=FALSE, fig.align="center", fig.show = "hold", out.width="100%", fig.cap='Exemplo de representação do agrupamento hierarquico. - Fonte: @han2011data'}
...
```{r kmeans-dataset, echo=FALSE ,fig.align="center", out.width="100%", fig.cap='Conjunto de dados utilizado como exemplo.'}
...
$$
<p align="justify">
Em que $x_i$ representa cada objeto do **Grupo 1**, $c_1$ representa o centróide deste mesmo grupo e $dist$ a medida de distância utilizada. Logo, o $SSE(grupo_1) \approx 1.9$, em outras palavras, o erro quadrático do **Grupó 1** é a soma da distância de todos os objetos em relação seu centróide. Então, para calcularmos o erro total dos grupos, basta fazermos um somatório de cada SSE:
</p>
...
```{r kmeans-elbow, echo=FALSE ,fig.align="center", out.width="100%", fig.cap='Método do cotovelo aplicado no conjunto de dados de exemplo.'}
...
```{r kmeans-silhueta, echo=FALSE ,fig.align="center", out.width="100%", fig.cap='Método de Silhueta aplicado no conjunto de dados de exemplo.'}
...
```{r hierarquico-dataset, echo=FALSE ,fig.align="center", out.width="100%", fig.cap='Conjunto de dados utilizado como exemplo.'}
...
```{r hierarquico-alg-0, echo=FALSE ,fig.align="center", out.width="100%", fig.cap='Conjunto de dados utilizado como exemplo.'}
...
<p align="justify">
Então, seguindo com o algoritmo do método aglomerativo, o próximo passo consiste na junção dos objetos em grupos, como foi dito, cada objeto inicialmente pertence ao seu próprio grupo. A etapa de junção dos objetos aos grupos segue a mesma ideia do que vimos no Kmeans, só que aqui estamos comparando dois objetos, e não os objetos com seus centróides. Então, os objetos mais próximos se juntam em um grupo, como apresentado na Figura \@ref(fig:hierarquico-alg-1). A junção
...
```{r hierarquico-alg-2, echo=FALSE ,fig.align="center", out.width="100%", fig.cap='Etapa de junção dos grupos.'}
